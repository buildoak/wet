...
> - **MECHANICAL** — tool_name matches CLI tools (Bash, git, npm, cargo, pytest, ls, cat, grep, find, make, docker, etc.) OR content is clearly command output. These compress deterministically — no replacement_text needed.
> - **AGENT_RETURN** — tool_name contains "Task" or "Agent" or "subagent", OR content is natural-language analysis/summary (not CLI output). These need LLM rewrite.
...
> - **PROTECTED** — error results, last 3 turns, image blocks, already-compressed tombstones (content contains `[compressed`), items under 50 tokens.
>
...
**Delete action:** If user marks a file read for deletion, it does NOT get summarized — it gets a minimal tombstone. Pass its ID to the compressor with `action=delete`. See Phase 4.
...
> POST these IDs with no replacement_text. Wet's Tier 1 handles them.
> ```bash
...
>
> For EACH id: fetch full content via `GET http://localhost:«WET_PORT»/_wet/inspect?full=1`, find the matching entry, then write a dense summary (under 150 tokens) preserving: key findings, decisions, file paths, metrics, open threads.
>
> POST all at once:
> ```bash
> curl -s -X POST "http://localhost:«WET_PORT»/_wet/compress" \
...
>
> These were file reads the user wants fully removed from context. Replace with a minimal tombstone — no content, no summary.
>
...
Report: new fill%, tokens_saved, items_compressed. Compare to Phase 1 numbers.
